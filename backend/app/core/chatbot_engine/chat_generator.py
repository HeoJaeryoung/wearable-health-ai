...
class ChatGenerator:
    ...
    def __init__(self):
        ...
        self.call_llm = self._call_openai
...
## 핵심 규칙
1. 캐릭터 말투와 특징적인 표현을 반드시 사용
2. **3-5문장으로 충실하게 응답**
3. 사용자의 감정에 공감하며 동기부여
4. 리스트/불릿 사용 금지, 자연스러운 대화체로
"""
...
"""
            ...
            base_instructions += """
## 일반 대화
- 캐릭터 특성을 살려 3-5문장으로 응답
- 사용자의 감정에 공감하고 격려/동기부여 제공
- 운동/건강 관련 조언을 자연스럽게 포함
"""
...
데이터 없음. 일반 조언을 2문장으로."""
                return self.call_llm(system, user_prompt, max_tokens=300)
