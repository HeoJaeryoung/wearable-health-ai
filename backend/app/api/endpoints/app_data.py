...
def get_latest_app_data(
    ...
    try:
        ...
        if not filtered_data:
            print(
                f"[WARN] {platform_filter} 플랫폼 데이터 없음, 전체 데이터에서 최신 선택"
            )
            filtered_data = metadatas

        # 3. 날짜 기준 최신순 정렬
        sorted_data = sorted(
            filtered_data, key=lambda x: x.get("date", ""), reverse=True
        )
...
def get_app_history(
    ...
    """
    print(
        f"[INFO] 앱 히스토리 조회: user_id={user_id}, watch_type={watch_type}, limit={limit}"
    )
    ...
    try:
        ...
        if watch_type:
            ...
            metadatas = [
                m
                for m in metadatas
                if platform_filter in m.get("source", "")
                or m.get("platform", "") == platform_filter
            ]
